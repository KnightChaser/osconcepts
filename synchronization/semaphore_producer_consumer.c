...
void* consumer(void* arg) {
    for (int i = 0; i < 10; i++) {
        ...
        sem_post(&mutexSemaphore);      // Unlock the buffer
        sem_wait(&emptySemaphore);      // Signal empty buffer
...
int main(int argc, char* argv[]) {
    pthread_t producerThread, consuemrThread;
    ...
    pthread_create(&consuemrThread, NULL, consumer, NULL);

    pthread_join(producerThread, NULL);
    pthread_join(consuemrThread, NULL);
